    ...
    tooltip: {
      custom: function({series, seriesIndex, dataPointIndex, w}) {
        return '<div style="padding: 5px;">' +
          // '<h1 style="font-size: 18px; color: #888E8F">'+mainData[dataPointIndex].toFixed(2)+'</h1>' +
          '</div>'
      },
    },
    stroke: {
  ...
  useEffect(() => {
    const init = async () => {
      ...
      if(filter === 1) {
        ...
        if(lastAPY !== undefined && !isNaN(lastAPY))
        ...
        userBalanceData = (data && data.userBalanceHistories !== undefined) ? data.userBalanceHistories : []
      ...
      let maxAPY = lastAPY, minAPY, maxTVL = lastTVL, minTVL, maxBalance, minBalance
      ...
      const yAxis = {
        ...
        labels: {
          style: { fontFamily: "Work Sans" },
          formatter: (val, ix) => numberWithCommas((filter === 1 ? round10(val, roundNum) : val).toFixed(filter === 1 ? 0 : len))
        }
      }
      ...
      setOptions({
          chart: {
            ...
            events: {
              scrolled: function(chartContext, { xaxis }) {
                console.log(new Date(xaxis.min).toLocaleDateString(), new Date(xaxis.max).toLocaleDateString())
              }
            }
          },
          grid: {
          ...
          tooltip: {
            ...
            custom: function({series, seriesIndex, dataPointIndex, w}) {
              return '<div style="padding: 5px;">' +
                '<h1 style="font-size: 12px; color: #888E8F">'+ (filter===1 ? "$" : "") + numberWithCommas(series[0][dataPointIndex].toFixed(filter === 1 ? 0 : len - 1)) + (filter === 0 ? "%" : "") + '</h1>' +
                '</div>'
            },
          },
          yaxis: yAxis,
