...
import LoopDeposit from './LoopDeposit'
import LoopWithdraw from './LoopWithdraw'
import { SwitchWrap, SwitchTab } from '../CLVault/style'

const LoopInteract = ({ data, connected, onRefresh }) => {
  ...
  return (
    <div style={{ padding: '15px' }}>
      ...
      {action === 'supply' ? (
        <LoopDeposit data={data} connected={connected} onRefresh={onRefresh} />
      ) : (
        <LoopWithdraw data={data} connected={connected} onRefresh={onRefresh} />
      )}
    </div>
  )
}

export default LoopInteract
