...
const WidoWithdrawFinalStep = ( { finalStep, setFinalStep, setWithdrawWido, setSelectTokenWith, symbol, useIFARM,  
  setClickedTokenId, setClickedVaultId, pickedToken, setPickedToken, token, unstakeBalance, setUnstakeBalance, slippagePercentage, fAssetPool, quoteValue} ) => {
  ...
  useEffect(() => {
    if(account && pickedToken.symbol !== "Select Token" && !new BigNumber(unstakeBalance).isEqualTo(0) && finalStep) {
  ...
  useEffect(()=>{
    if(quoteValue) {
      const getQuoteResult = async () => {
        try{
          const fromInfoTemp = quoteValue && formatNumberWido(fromWei(quoteValue.fromTokenAmount, token.decimals || token.data.lpTokenData.decimals), WIDO_BALANCES_DECIMALS) + (quoteValue.fromTokenUsdPrice === null ? "" : " ($" + formatNumberWido(fromWei(quoteValue.fromTokenAmount * quoteValue.fromTokenUsdPrice, token.decimals || token.data.lpTokenData.decimals), WIDO_BALANCES_DECIMALS) + ")")
          const toInfoTemp = quoteValue && formatNumberWido(fromWei(quoteValue.toTokenAmount, pickedToken.decimals), WIDO_BALANCES_DECIMALS) + (quoteValue.toTokenUsdPrice === null ? "" : " ($" + formatNumberWido(fromWei(quoteValue.toTokenAmount * quoteValue.toTokenUsdPrice, pickedToken.decimals), WIDO_BALANCES_DECIMALS) + ")")
  ...
  const onClickApprove = async () => {
    ...
    try {
      ...
      if(!new BigNumber(allowance).gte(unstakeBalance)) {
        const amountToApprove = new BigNumber(unstakeBalance)
          .minus(allowance)
  ...
  return (
    <SelectTokenWido show={finalStep} backColor={backColor} borderColor={borderColor}>
      <NewLabel display={"flex"} justifyContent={"space-between"} marginBottom={"15px"} weight={"500"} size={"14px"} height={"18px"} color={"#1F2937"} align={"center"}>
      ...
      <NewLabel marginBottom={"20px"}>
        <WidoSwapToken img={Swap1WithIcon} name={fromInfo} value={useIFARM ? symbol : token.balance} />
      ...
      <NewLabel size={"16px"} height={"21px"} weight={600}>
        <Buttons show={approveValue} onClick={()=>{ onClickApprove() }}>
          {
            approveValue === 2 ?
              <>
        ...
        <ExecuteButton approve={approveValue} execute={executeValue} onClick={()=>{ onClickExecute() }}>
        {
          executeValue === 1 ? 
          <>
            Withdraw
        ...
        <CloseButton show={executeValue} onClick={()=>{ onClickClose() }}>
